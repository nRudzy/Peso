...
class TestIntegrationWorkflow:
    ...
    def test_complete_user_registration_workflow(self, client, db_session):
        ...
        response = client.post("/api/v1/weight-entries", json=weight_data, headers=headers)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_weight_tracking_workflow(self, client, auth_headers):
        ...
        for weight, date in zip(weights, dates):
            ...
            response = client.post("/api/v1/weight-entries", json=weight_data, headers=auth_headers)
            assert response.status_code == status.HTTP_200_OK
        
        # 2. Get weight entries with pagination
        response = client.get("/api/v1/weight-entries?page=1&limit=3", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        ...
        response = client.get("/api/v1/weight-entries/progress/30", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_error_handling_workflow(self, client, auth_headers):
        ...
        response = client.put("/api/v1/weight-entries/99999", json=update_data, headers=auth_headers)
        assert response.status_code == status.HTTP_404_NOT_FOUND
        ...
        response = client.post("/api/v1/weight-entries", json=invalid_data, headers=auth_headers)
    ...
    def test_pagination_workflow(self, client, db_session, auth_headers):
        ...
        # Test pagination
        response = client.get("/api/v1/weight-entries?page=1&limit=10", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        ...
        # Test second page
        response = client.get("/api/v1/weight-entries?page=2&limit=10", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert len(data["data"]) == 10
