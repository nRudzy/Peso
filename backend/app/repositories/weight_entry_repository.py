...
class WeightEntryRepository(
    BaseRepository[WeightEntry, WeightEntryCreateDB, WeightEntryUpdate]
):
    ...
    def get_by_user_and_id(self, user_id: int, entry_id: int) -> Optional[WeightEntry]:
        ...
        return (
            self.db.query(WeightEntry)
            .filter(and_(WeightEntry.user_id == user_id, WeightEntry.id == entry_id))
            .first()
        )
    ...
    def count_by_user(self, user_id: int) -> int:
        ...
        return self.db.query(WeightEntry).filter(WeightEntry.user_id == user_id).count()
